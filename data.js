...
const STORAGE_KEYS = {
  DATA: 'SEMICON_HUB_DATA_V10', // V10: 10대 핵심 기술/용어 개념 카드 및 안정적 fallback 보장
  ...
  concepts: [
    {
      ...
      icon: "🧠",
      summary: "정보를 '저장'하는 메모리와 정보를 '연산·처리'하는 뇌 역할의 핵심 구분입니다.",
      details: [
        "<b>메모리 반도체</b>: DRAM, NAND Flash 등 데이터 기억 전담 (한국 삼성전자, SK하이닉스 세계 시장 주도)",
        "<b>시스템 반도체</b>: CPU, GPU, AP, NPU 등 데이터 연산/처리 전담 (NVIDIA, Intel, Apple, Qualcomm)",
        "<b>파운드리(Foundry)</b>: 팹리스의 도면을 위탁받아 전문 생산해주는 공장 (TSMC, 삼성 파운드리)"
      ]
    },
    {
      id: "eight-processes",
      category: "제조공정",
      title: "반도체 8대 핵심 제조 공정 프로세스",
      icon: "🏭",
      summary: "실리콘 모래에서 실물 첨단 반도체 칩이 완성되기까지의 8단계 전/후공정입니다.",
      details: [
        "1~2. <b>웨이퍼 & 산화</b>: 실리콘 인곳 원기둥을 자르고 절연 산화막을 형성",
        "3~4. <b>포토(노광) & 식각</b>: EUV 빛으로 미세 회로를 그리고 필요없는 부위를 화학 깎아내기",
        "5~6. <b>박막 증착 & 금속 배선</b>: 원자층 보호막을 입히고 신호 전달 금속 선 연결",
        "7~8. <b>EDS 테스트 & 패키징</b>: 불량 칩 선별 검사를 거쳐 밀봉 및 전극 가공 완료"
      ]
    },
    {
      id: "hbm-intro",
      ...
      icon: "⚡",
      summary: "DRAM 칩 여러 개를 수직 적층하여 데이터 이동 병목을 해소한 AI 초고속 메모리입니다.",
      details: [
        "<b>TSV (실리콘 관통 전극)</b>: 미세 구멍을 뚫어 상하 칩을 8~12층 이상 수직 결합",
        "<b>AI 가속기 필수 핵심 부품</b>: NVIDIA H100, B200 등 대규모 AI 데이터센터에 탑재",
        "<b>후공정 기술 혁신</b>: SK하이닉스 MR-MUF / NCF 소재 기술로 열 방출과 수율 극대화"
      ]
    },
    {
      id: "ecosystem-structure",
      category: "산업구조",
      title: "반도체 산업 생태계 분업 구조 (IDM, 팹리스, 파운드리, OSAT)",
      icon: "🌐",
      summary: "설계부터 가공, 패키징까지 철저히 전문화된 글로벌 반도체 분업 시스템입니다.",
      details: [
        "<b>IDM (종합 반도체 기업)</b>: 설계, 생산, 패키징 전 과정을 자체 수행 (삼성전자, Intel)",
        "<b>팹리스 (Fabless)</b>: 공장 없이 100% 회로 설계에만 집중 (엔비디아, 퀄컴, AMD)",
        "<b>파운드리 (Foundry)</b>: 팹리스 도면을 위탁받아 첨단 미세 공정으로 생산 전담 (TSMC)",
        "<b>OSAT (후공정 전문)</b>: 웨이퍼 가공 완료 후 패키징 및 최종 테스트 외주 전담"
      ]
    },
    {
      id: "ultrafine-nodes",
      category: "공정기술",
      title: "초미세 공정 혁신 (High-NA EUV & GAA 트랜지스터)",
      icon: "🔬",
      summary: "3나노 및 2나노 이하 미세 회로를 구현하기 위한 광학 및 3D 트랜지스터 혁신입니다.",
      details: [
        "<b>High-NA EUV 노광 장비</b>: 렌즈 개구수(0.55)를 올려 2나노 이하 회로 패턴 정밀 작형",
        "<b>GAA (Gate-All-Around)</b>: 게이트가 전류 채널 4개 면을 감싸 누설 전류 최소화",
        "<b>BSPDN (후면 전력 공급)</b>: 전원 배선을 웨이퍼 뒷면으로 돌려 전력 효율/회로 밀도 상승"
      ]
    },
    {
      id: "advanced-packaging",
      category: "후공정",
      title: "첨단 후공정 & 칩렛 (Advanced Packaging & Chiplet)",
      icon: "📦",
      summary: "단일 칩 미세화의 한계를 극복하기 위해 다종 칩을 하나로 묶는 2.5D/3D 적층 기술입니다.",
      details: [
        "<b>2.5D/3D 이종 집적</b>: GPU와 HBM을 인터포저 기판 위에 결합 (예: TSMC CoWoS)",
        "<b>Chiplet (칩렛)</b>: 대형 칩을 기능별 소형 칩으로 분할 생산 후 하나로 패키징해 비용 절감",
        "<b>FOWLP</b>: PCB 기판 없이 웨이퍼 수준에서 직접 전극을 배치하여 얇은 두께 구현"
      ]
    },
    {
      id: "nextgen-memory",
      category: "차세대기술",
      title: "차세대 메모리 규격 (CXL & PIM)",
      icon: "🚀",
      summary: "서버 메모리 용량을 무한 확장하고 메모리 내부에서 직접 연산하는 미래형 메모리입니다.",
      details: [
        "<b>CXL (Compute Express Link)</b>: CPU-메모리 통로 확장으로 서버 D램 용량을 테라바이트급 확장",
        "<b>PIM (Processing-In-Memory)</b>: 메모리 내부에서 연산을 처리해 CPU 데이터 전송 병목 제거",
        "<b>LPDDR5X & GDDR7</b>: 온디바이스 AI 및 고성능 그래픽 카드를 위한 초저전력·초고속 D램"
      ]
    },
    {
      id: "ondevice-ai-npu",
      category: "시스템반도체",
      title: "온디바이스 AI & NPU (신경망 처리 장치)",
      icon: "📱",
      summary: "클라우드 연결 없이 기기 자체 단말에서 실시간 AI를 구동하는 전용 칩입니다.",
      details: [
        "<b>NPU (Neural Processing Unit)</b>: 인공지능 딥러닝 병렬 연산에 최적화된 저전력 칩",
        "<b>온디바이스 AI</b>: 스마트폰, PC에서 오프라인 상태로 실시간 통번역, 이미지 생성 구동",
        "<b>AP (Application Processor)</b>: CPU, GPU, NPU, 통신 모뎀을 칩 하나로 묶은 모바일 뇌"
      ]
    },
    {
      id: "power-semiconductor",
      category: "차세대화합물",
      title: "차세대 전력 반도체 (SiC & GaN 화합물)",
      icon: "⚡",
      summary: "전기차, 충전기, 데이터센터 등 고전압·고온 환경에서 전력 효율을 높이는 기술입니다.",
      details: [
        "<b>SiC (탄화규소)</b>: 실리콘 대비 10배 높은 고전압 견딤성으로 전기차 인버터 탑재",
        "<b>GaN (질화갈륨)</b>: 초고속 스위칭 특성으로 초소형 고속 충전기 및 고주파 장비 적용",
        "<b>전력 손실 절감</b>: 에너지 전환 효율을 끌어올려 글로벌 탄소 중립 및 고효율 구현"
      ]
    },
    {
      id: "semicon-materials-yield",
      category: "재료/기초",
      title: "반도체 핵심 재료 & 수율 (Wafer, PR, Yield)",
      icon: "🧪",
      summary: "반도체 제조의 원자재 및 생산 성공률을 나타내는 핵심 기본 지표입니다.",
      details: [
        "<b>웨이퍼 (Wafer) & 인곳</b>: 고순도 모래(실리콘) 원기둥을 얇게 잘라낸 기판 원판",
        "<b>포토레지스트 (PR) & 식각액</b>: 회로 패턴을 쬐어 만드는 감광액 및 화학 깎아내기 물질",
        "<b>수율 (Yield)</b>: 웨이퍼 한 장에서 양품(결함 없는 칩)의 백분율로 기술력과 수익성 상징"
      ]
    }
  ],
  glossary: [
