...
const STORAGE_KEYS = {
  DATA: 'SEMICON_HUB_DATA_V9', // V9로 갱신하여 8대 기본 개념, 5대 언론사(25개 기사) 및 50개 용어 보장
  ...
  concepts: [
    {
      ...
      details: [
        ...
        "<b>시스템 반도체</b>: CPU, GPU, AP, NPU 등 생각하고 계산하는 뇌 역할을 합니다. 대표 기업으로 NVIDIA, Intel, Apple 등이 있습니다.",
    ...
    },
    {
      ...
      details: [
        "<b>개념 및 목적</b>: AI 연산 시 막대한 데이터 이동 병목 현상을 해결하기 위해 만들어졌습니다.",
        "<b>핵심 기술</b>: TSV(실리콘 관통 전극) 기술로 칩에 미세한 구멍을 뚫어 수직으로 연결합니다.",
        "<b>시장 적용</b>: NVIDIA AI 가속기(H100, B200 등)의 필수 핵심 부품으로 글로벌 수요가 폭증하고 있습니다."
      ]
    },
    {
      id: "eight-processes",
      ...
      icon: "🏭",
      summary: "모래(실리콘)에서 첨단 반도체 칩이 탄생하기까지의 8가지 핵심 전/후공정입니다.",
      details: [
        "1. <b>웨이퍼 제조</b>: 실리콘(모래)을 녹여 원기둥(인곳)을 만들어 얇게 썰어냅니다.",
        "2. <b>산화 공정</b>: 웨이퍼 표면에 절연 보호막(산화막)을 형성합니다.",
        "3. <b>포토 공정(노광)</b>: 빛(EUV/DUV)을 이용해 회로 패턴을 웨이퍼에 그립니다.",
        "4. <b>식각 공정(Etching)</b>: 불필요한 부위를 화학/물리적으로 깎아내 회로만 남깁니다.",
        "5. <b>박막/증착 공정</b>: 원자층 단위의 얇은 막을 씌워 전기적 특성을 갖게 합니다.",
        "6. <b>금속 배선 공정</b>: 신호가 통하도록 미세 금속 선을 연결합니다.",
        "7. <b>EDS(테스트) 공정</b>: 전기적 시험으로 불량 칩을 선별합니다.",
        "8. <b>패키징 공정</b>: 칩을 보호 케이스에 넣고 외부 기기와 연결되도록 가공합니다."
      ]
    },
    {
      id: "ecosystem-structure",
      category: "산업구조",
      title: "반도체 산업 생태계 (IDM, 팹리스, 파운드리, OSAT)",
      icon: "🌐",
      summary: "설계부터 생산, 패키징까지 전문화된 반도체 산업 분업 체계입니다.",
      details: [
        "<b>IDM (종합 반도체 기업)</b>: 설계부터 생산, 패키징까지 전 과정을 자체 수행 (예: 삼성전자, Intel).",
        "<b>팹리스 (Fabless)</b>: 공장 없이 반도체 회로 설계에만 집중 (예: 엔비디아, 퀄컴, AMD).",
        "<b>파운드리 (Foundry)</b>: 팹리스의 도면을 받아 위탁 생산만 전담 (예: TSMC, 삼성 파운드리).",
        "<b>OSAT (후공정 전문)</b>: 가공된 웨이퍼의 패키징 및 테스트를 전문 외주 처리하는 기업."
      ]
    },
    {
      id: "ultrafine-nodes",
      category: "공정기술",
      title: "초미세 공정 혁신 (EUV 노광 & GAA 트랜지스터)",
      icon: "🔬",
      summary: "3나노 이하 미세 회로를 구현하기 위한 광학 장비 및 3D 구조 트랜지스터 혁신입니다.",
      details: [
        "<b>High-NA EUV 노광</b>: 파장 13.5nm 극자외선으로 2나노 이하 회로 패턴을 렌즈로 그리는 첨단 장비.",
        "<b>GAA (Gate-All-Around)</b>: 게이트가 전류 채널 4개 면을 모두 감싸 누설 전류를 극적으로 줄이는 3D 구조.",
        "<b>BSPDN (후면 전력 공급)</b>: 배선을 웨이퍼 뒷면으로 돌려 전력 효율과 회로 밀도를 극대화."
      ]
    },
    {
      id: "advanced-packaging",
      category: "후공정",
      title: "첨단 후공정 & 패키징 (Advanced Packaging & TSV)",
      icon: "📦",
      summary: "단일 칩 미세화의 한계를 극복하기 위해 다종 칩을 하나로 묶는 2.5D/3D 적층 기술입니다.",
      details: [
        "<b>2.5D/3D 이종 집적</b>: GPU와 HBM 메모리를 인터포저 기판 위에 하나로 패키징 (예: TSMC CoWoS).",
        "<b>Chiplet (칩렛)</b>: 커다란 칩을 기능별로 조각내어 개별 생산 후 수직/수평으로 결합하여 수율 향상.",
        "<b>MR-MUF / NCF</b>: HBM 칩 적층 시 열 방출과 보호를 위한 획기적인 후공정 소재/액체 주입 기술."
      ]
    },
    {
      id: "nextgen-memory",
      ...
      icon: "🚀",
      summary: "메모리 용량 한계를 뛰어넘고 연산까지 직접 수행하는 미래형 메모리 기술입니다.",
      details: [
        "<b>CXL (Compute Express Link)</b>: CPU와 메모리 간 통로를 획기적으로 확장해 서버 메모리 용량을 무한 확장.",
        "<b>PIM (Processing-In-Memory)</b>: 메모리 내부에서 직접 데이터 연산을 수행해 폰노이만 병목현상 제거.",
        "<b>LPDDR5X & GDDR7</b>: 온디바이스 AI 및 고성능 그래픽용 초저전력·초고속 차세대 DRAM."
      ]
    },
    {
      id: "ondevice-ai-npu",
      ...
      icon: "📱",
      summary: "클라우드 연결 없이 기기 자체에서 실시간 인공지능을 구동하는 초저전력 시스템 칩입니다.",
      details: [
        "<b>NPU (Neural Processing Unit)</b>: 인공지능 딥러닝 병렬 연산에 최적화된 저전력 하드웨어 아키텍처.",
        "<b>온디바이스 AI</b>: 스마트폰, PC, 자율주행 차에서 오프라인 상태로 텍스트/이미지/통역 AI 처리.",
        "<b>AP (Application Processor)</b>: CPU, GPU, NPU, 통신 모뎀을 하나로 모은 모바일의 뇌 칩."
      ]
    }
  ],
  glossary: [
